...
File dataFile;
int totalweight,savedDay,savedMonth,savedYear,savedHour,savedMinute;
...
if (!SD.begin(53)) {
  ...
  //----------------------------------------------------------------------------------------- This is my code here, at the end you get the day input
...
void loop() {
  ...
  if(savedDay!= a && savedHour<d ){
    if(magikNumber<ARRAY_SIZE(date_max)-1){
    magikNumber += 1;
    savedDay +=1;
    ...
    if(dataFile){
      ...
      Serial.println("i dont have enough data...");
    }
  }
  totalweight=1000;                                   //Update your weight HERE!

int i = 1;
float last;
float x=totalweight/i;
 Serial.println(x);
  Serial.println(date_min[magikNumber-1]);
   Serial.println(date_max[magikNumber-1]);
while(date_max[magikNumber-1]<x){
 
 i+=1;
 last = x;
 x=totalweight/i;
}
  if(abs((date_min[magikNumber-1]+ date_max[magikNumber-1])/2-last)<abs((date_min[magikNumber-1]+ date_max[magikNumber-1])/2-x  )){
  
      Serial.print("old guess: ");
Serial.println(abs((date_min[magikNumber-1]+ date_max[magikNumber-1])/2-last));
...
Serial.println(abs((date_min[magikNumber-1]+ date_max[magikNumber-1])/2-x  ));
  ...
  if (dataFile) {Serial.println("Data logged with ");
